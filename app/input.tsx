...
const districts = {
  ...
  '경기도': [
    ...
    '이천시', '구리시', '의왕시', '포천시', '양평군', '여주시', '동두천시', '가평군', '과천시', '연천군'
  ]
};

// 학력 데이터
const educationList = [
  '제한 없음', '고졸 미만', '고교 재학', '고졸 예정', '고교 졸업', 
  ...
  const handleNextStep = () => {
    Alert.alert(
      ...
      [
        ...
        { text: "확인", onPress: () => console.log("다음 페이지 이동") }
      ]
    );
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        {/* 소득 구간 - 이 부분의 div를 View로 수정했습니다 */}
        <View style={styles.inputGroup}>
          <Text style={styles.label}>소득 구간 (중위소득 기준)</Text>
        ...
        <View style={styles.inputGroup}>
          ...
          <View style={styles.row}>
            {['재직', '구직', '프리랜서'].map((status) => (
              <TouchableOpacity key={status} style={[styles.choiceBtnSmall, jobStatus === status && styles.activeBtn]} onPress={() => setJobStatus(status)}>
        ...
        <View style={styles.inputGroup}>
          ...
          <TouchableOpacity style={styles.dropdown} onPress={() => setModalType('edu')}>
      ...
      <SelectorModal visible={modalType === 'edu'} title="학력 선택" data={educationList} onSelect={(val) => { setEducation(val); setModalType(null); }} onClose={() => setModalType(null)} />
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  label: { fontSize: 15, fontWeight: '700', color: '#555', marginBottom: 12 },
  ...
  textInput: { flex: 1, paddingVertical: 15, fontSize: 16, color: '#333', fontWeight: '600' },
  ...
  choiceBtnSmall: { flex: 1, backgroundColor: '#FFF', padding: 18, borderRadius: 14, borderWidth: 1, borderColor: '#EEE', alignItems: 'center', marginHorizontal: 4 },
  ...
  nextButton: { backgroundColor: '#67B292', padding: 20, borderRadius: 30, alignItems: 'center', marginTop: 30 },
  nextButtonText: { color: '#FFF', fontSize: 18, fontWeight: 'bold' },
  
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center' },
  modalContent: { width: '80%', backgroundColor: '#FFF', borderRadius: 25, padding: 20 },
  modalTitle: { fontSize: 18, fontWeight: '800', color: '#111', marginBottom: 15, textAlign: 'center' },
