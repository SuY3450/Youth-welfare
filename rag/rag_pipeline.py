...
# ══════════════════════════════════════════════════════════
#  [신규] PDF OCR → 온보딩 자동 입력
# ══════════════════════════════════════════════════════════
def extract_user_info_from_pdf(pdf_path: str) -> dict:
    """
    PDF 파일에서 텍스트 추출 → LLM이 파싱하여 온보딩 정보 자동 생성.
    소득증명서, 주민등록등본 등을 첨부하면 나이/지역/소득 자동 입력.

    Args:
        pdf_path: PDF 파일 경로

    Returns:
        dict: 추출된 사용자 정보 (user_info 형식)
    """
    extracted_text = ""
    try:
        import pdfplumber
        with pdfplumber.open(pdf_path) as pdf:
            for page in pdf.pages[:5]:
                ...
                if text:
                    extracted_text += text + "\n"
        print(f"📄 PDF 텍스트 추출 완료 (pdfplumber): {len(extracted_text)}자")
    except ImportError:
        try:
            ...
            for page in reader.pages[:5]:
                ...
                if text:
                    extracted_text += text + "\n"
            print(f"📄 PDF 텍스트 추출 완료 (PyPDF2): {len(extracted_text)}자")
        except ImportError:
            print("⚠️ pdfplumber 또는 PyPDF2가 설치되어 있지 않습니다.")
            return {}

    if not extracted_text.strip():
        print("⚠️ PDF에서 텍스트를 추출할 수 없습니다 (스캔 이미지 PDF일 수 있음)")
        return {}

    try:
        llm = get_llm()
        prompt = f"""아래는 사용자가 첨부한 PDF 문서에서 추출한 텍스트입니다.
이 문서에서 청년 복지 정책 신청에 필요한 사용자 정보를 추출하세요.

[추출된 텍스트]
{extracted_text[:3000]}

아래 항목을 추출하세요. 문서에 없는 정보는 빈 문자열로 답하세요.
JSON 형식으로만 응답 (마크다운 없이):
{{{{
  "age": "만 나이 (숫자만). 생년월일 있으면 현재 기준 계산",
  "region": "광역시도 (예: 서울특별시)",
  "sub_region": "시/군/구 (예: 서초구)",
  "income_level": "소득 구간 (50% 이하 / 50~100% / 100~150% / 150% 초과)",
  "employment": "취업 상태 (미취업 / 근무 / 프리랜서)",
  "education": "최종 학력",
  "housing": "주거 형태 (월세 자취 / 전세 / 자가 / 기숙사 / 부모님 동거)",
  "document_type": "문서 종류 (예: 소득증명서)",
  "extracted_fields": ["추출 성공한 필드명 리스트"]
}}}}"""
        response = llm.invoke([HumanMessage(content=prompt)])
        result = json.loads(response.content.replace("```json","").replace("```","").strip())

        if result.get("age"):
            try: result["age"] = int(result["age"])
            except: result["age"] = ""

        doc_type = result.pop("document_type", "")
        extracted = result.pop("extracted_fields", [])
        print(f"📋 문서 종류: {doc_type}")
        print(f"✅ 추출 성공: {', '.join(extracted) if extracted else '없음'}")
...
def summarize_pdf(pdf_path: str) -> str:
    ...
    try:
        ...
        try:
            ...
            return "⚠️ pdfplumber 또는 PyPDF2가 설치되어 있지 않습니다."
    ...
    try:
        llm = get_llm()
        prompt = f"""아래 PDF 문서 내용을 3~5줄로 핵심만 요약하세요.
